...
type StoredClient = {
   ...
   * Primarily used for query batching, but can also be used for logging, etc.
   */
...
const als = new AsyncLocalStorage<StoredClient | undefined>();
...
export async function stickyClient<T>(fn: () => Promise<T>): Promise<T> {
  ...
  return await runWithStoredClient({ transactionId: null, client }, fn);
}
